...
void Queue::enqueue(vertex* node){
...
void Queue::sorting(){
    ...
    for(i=head+1;i<tail;i++){
        vertex *index=store[i].v;
        j=i;
        while(j>head && store[j-1].v->F > index->F){
            store[j]=store[j-1];
            j--;
        }
        store[j].v=index;
...
void Astar::searchpath(string start,string destinatioin){
    ...
    while(open.count!=0){
        ...
        if (current->flag==true){
            ...
            cout<<"The path is "<< close.dequeue()->name;
            traceback(current);
            cout<<endl;
        ...
        for(int i=0;i<current->adj.size();i++){
            bool deter=close.check(current->adj[i].v);
            if (deter==true){
        ...
        for(int i=0; i<current->adj.size();i++){
            cout<<current->adj[i].v->name<<endl;
            cout<<current->adj[i].v->G<<endl;
        }
        open.sorting();
    }
...
void Astar::deletecity(string city){
    ...
    if(det==NULL){
        ...
        while(det->adj.size()!=0){
            int i=det->adj.size();
            cout<<det->adj[i-1].v->name<<endl;
            deletepath(det->adj[i-1].road,det->name,det->adj[i-1].v->name);
        }
...
int main(){
    ...
    cout<<"===================================="<<endl;
    cout<<"The pre-installed map has following routes: "<<endl;
    cout<<endl;
    int choice=0;
    A.presetup();
    cout<<endl;
   /*vertex *det= &A.vertices[2];
    for(int i=0;i<det->adj.size();i++){
        cout<<det->adj[i].v->name<<endl;
    }*/
